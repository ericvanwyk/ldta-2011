...
some type. We heavily utilize this property --- actually there are not a single 
...
or \emph{open}, type. Then transformers for some types can be combined by a certain primitive
...
declared. Another feature of polymorphic variant types is that they can be open --- in certain
context only some of their constructors may be known.

Polymorphic variants in Objective Caml can be used to express the ultimate solution
for the \emph{expression problem}~\cite{Exproblem, PVReuse} --- that is, the problem of
modular and type-safe implementation of extending data structure processing. 
...
later. Note also that these functions are completely independent; thus, they can be implemented 
in a different compilation units or provided at arbitrary moments of program execution.
...
let (++) left right = 
  fun ext s -> 
    left (fun self z -> right (fun _ z -> ext self z) z) 
         s
\end{lstlisting}

This combinators takes extensible transformers for some open types and provides extensible transformer 
...
%let evalAll s = evalIdent s ++ evalMult ++ evalAddSub 

%let evalSimple s = evalAddSub ++ evalIdent s
%let evalMult   s = evalSimple s ++ evalMult
